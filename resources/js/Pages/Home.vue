    ...
    <MainLayout>
        <div class="flex flex-col mb-6">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold tracking-tight">Explorar Destinos</h1>
                    <p class="text-sm text-base-content/50 mt-1">Descubra os melhores pontos turísticos do Brasil</p>
                </div>
                <button
                    @click="sidebarAberta = true"
                    class="btn btn-ghost btn-sm gap-1.5 lg:hidden relative"
                    title="Abrir filtros"
                >
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z" />
                    </svg>
                    <span v-if="contagemFiltros > 0" class="badge badge-xs badge-primary badge-outline absolute -top-1 -right-1">{{ contagemFiltros }}</span>
                </button>
            </div>

            <div v-if="!loading && pontos.data.length" class="mt-4">
                <StoriesBar :stories="destinosEmAlta" @open-story="abrirStory" />
                <DestinosCarrossel :destinos="pontos.data" />
            </div>
        </div>
        ...
        <div class="flex gap-0 relative">
                    ...
                    <div class="flex-1 overflow-y-auto px-5 py-4 space-y-5">
                        <div>
                            <span class="text-[11px] font-semibold text-base-content/40 uppercase tracking-wider flex items-center gap-2 mb-4">
                                ...
                                Filtros
                                <span v-if="contagemFiltros > 0" class="badge badge-xs badge-soft badge-primary">{{ contagemFiltros }}</span>
                            </span>

                            <SearchBar :search="filtros.search" @update:search="buscar" />

                            <div class="mt-5">
                                <LocationFilter :uf-ativo="filtros.uf" />
                            </div>
            ...
            >
                ...
                <span v-if="contagemFiltros > 0 && !sidebarDesktopAberta" class="badge badge-xs badge-primary">{{ contagemFiltros }}</span>
            </button>
            ...
            <div class="flex-1 min-w-0">
                ...
                <template v-else-if="pontos.data.length === 0">
                    <div class="text-center py-20 max-w-md mx-auto">
                        ...
                        <h3 class="text-lg font-semibold text-base-content/60">Nenhum resultado encontrado</h3>
                        <p class="text-sm text-base-content/40 mt-2">Tente ajustar sua busca ou explorar outras categorias.</p>
                        <Link href="/" class="btn btn-primary btn-sm mt-6">Ver todos os destinos</Link>
                    </div>
                </template>
...
<Pagination :links="pontos.links" />
         ...
         <StoryModal :story="storySelecionado" :open="storyModalAberto" @close="fecharStory" />
     </MainLayout>
 </template>
...
import DestinosCarrossel from '../Components/DestinosCarrossel.vue'
...
const props = defineProps({
    ...
    cidades: { type: Array, default: () => [] },
    ufs: { type: Array, default: () => [] },
    filtros: {
        type: Object,
        default: () => ({ search: '', categoria: '', cidade: '', sort: '', uf: '' }),
    },
})
...
const storySelecionado = ref(null)
const storyModalAberto = ref(false)
...
const temFiltro = computed(() => contagemFiltros.value > 0)

const destinosEmAlta = computed(() => {
    return props.pontos.data
        .filter(p => p.avaliacao >= 4.5)
        .sort((a, b) => b.curtidas - a.curtidas)
        .slice(0, 10)
        .map(p => ({
            ...p,
            nome_curto: p.nome.length > 10 ? p.nome.substring(0, 8) + '…' : p.nome,
        }))
})
...
function abrirStory(story) {
    storySelecionado.value = story
    storyModalAberto.value = true
}

function fecharStory() {
    storyModalAberto.value = false
    storySelecionado.value = null
}
</script>
